...
async function main() {
  ...
  async function getRegistrarId(): Promise<number> {
    const [rows] = await q(
      "SELECT id FROM users WHERE role = 'registrar' AND status = 'active' LIMIT 1"
    );
    if ((rows as any[]).length === 0)
      throw new Error('No active registrar user.');
    return (rows as any[])[0].id;
  }

  async function sectionExists(name: string): Promise<boolean> {
    const [r] = await q('SELECT id FROM sections WHERE name = ? LIMIT 1', [
      name
    ]);
  ...
  async function addStudent(
    ...
    if (await studentExists(lrn)) {
      const [r] = await q('SELECT id FROM students WHERE lrn = ? LIMIT 1', [
        lrn
      ]);
      return (r as any[])[0].id;
    }
  ...
  async function addGrades(
    ...
    const [existing] = (await q(
      'SELECT COUNT(*) AS n FROM grades WHERE enrollment_id = ?',
      [enrollmentId]
    )) as any[];
    if (existing[0].n > 0) return; // already seeded for this enrollment
    const [subs] = await q(
      'SELECT id, name FROM subjects WHERE grade_level = ? AND is_active = 1',
      [gradeLevel]
    );
    const values: any[] = [];
  ...
  console.log('\nâ€” Adding JHS Regular students (Random Distribution)');
  type S = {
    display: string;
    lrn: string;
    name: string;
  ...
  for (const s of regularJHS) {
    ...
    const en = await addEnrollment(id, SY, 'regular', {
    ...
    console.log(
      `  G${s.grade} ${s.name.padEnd(18)} ${s.display} (GA ~${s.avg})`
    );
  }

  console.log('\nâ€” Adding STE applicants (Placement)');
  const steApplicants: Array<
    S & {
      exam: boolean | null;
  ...
  for (const s of steApplicants) {
    ...
    console.log(
      `  G${s.grade} ${s.name.padEnd(18)} ${s.display} exam=${s.exam} interview=${s.interview} ${s.over ? '(deliberate low grade)' : ''}`
    );
  }

  console.log('\nâ€” Adding SPFL applicants (Placement)');
  const spflApplicants: Array<
    S & {
      exam: boolean | null;
      interview: boolean | null;
      over?: Record<string, number>;
  ...
  console.log('\nâ€” Adding returning G11->G12 students (Carry-Over)');
  const g11Ste = (
    await q("SELECT id FROM sections WHERE name = '11-Maasahan' LIMIT 1")
  )[0] as any[];
  const g11Reg = (
    await q("SELECT id FROM sections WHERE name = '11-Matatag' LIMIT 1")
  )[0] as any[];
  ...
  for (const s of returning) {
    ...
    const en = await addEnrollment(id, SY, s.program, {
      enrolledBy: registrarId,
      examPassed: null,
      interviewPassed: null
    });
    ...
    console.log(
      `  G12 ${s.name.padEnd(18)} ${s.display} ${s.program} ${s.avg ? `GA ~${s.avg}` : '(no grades)'}`
    );
  }
