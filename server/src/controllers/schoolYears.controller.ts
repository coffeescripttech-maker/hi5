...
export async function createSchoolYear(
  ...
  try {
    ...
    if (!sy_label) {
      res
        .status(400)
        .json({ error: "sy_label is required (e.g. '2026-2027' or '2026')." });
      return;
    }
    ...
    if (!/^\d{4}[-–]\d{4}$/.test(trimmed) && !/^\d{4}$/.test(trimmed)) {
      res.status(400).json({
        error:
          "School year must be a school-year range (e.g. '2026-2027') or a calendar year (e.g. '2026')."
      });
      return;
    }
    ...
    if (existing.length > 0) {
      res
        .status(409)
        .json({ error: `School year "${trimmed}" already exists.` });
      return;
    }
    ...
    const countRows = await query<RowDataPacket[]>(
      'SELECT COUNT(*) AS c FROM school_years'
    );
    ...
    await logActivity(
      req.user!.userId,
      `Created school year "${trimmed}"`,
      'school_years',
      result.insertId
    );

    const newYear = await query<RowDataPacket[]>(
...
export async function updateSchoolYear(
  ...
  try {
    ...
    if (sy_label !== undefined) {
      fields.push('sy_label = ?');
      params.push(sy_label);
    }
    if (enrollment_open !== undefined) {
      fields.push('enrollment_open = ?');
      params.push(enrollment_open ? 1 : 0);
    }
 ...
 */
export async function setCurrentSchoolYear(
  req: Request,
  res: Response
): Promise<void> {
  let connection;
  try {
    ...
    await connection.execute('UPDATE school_years SET is_current = 0');
    await connection.execute(
      'UPDATE school_years SET is_current = 1 WHERE id = ?',
      [id]
    );
    ...
    await logActivity(
      req.user!.userId,
      `Set "${existing[0].sy_label}" as current school year`,
      'school_years',
      id
    );
...
export async function archiveSchoolYear(
  ...
  try {
    ...
    if (current.is_current !== 1) {
      res
        .status(400)
        .json({ error: 'Only the current school year can be archived.' });
      return;
    }
    ...
    if (!nextLabel) {
      res
        .status(400)
        .json({ error: 'Could not determine the next school year label.' });
      return;
    }
