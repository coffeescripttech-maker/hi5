...
const newStudentSchema = z.object({
  ...
  birthdate: z.string()
    ...
    .refine(val => new Date(val) <= new Date(), "Birthdate cannot be in the future")
    .refine(val => {
  ...
  guardian: z.string().min(1, "Guardian name is required").min(2, "Guardian name must be at least 2 characters"),
  contact: z.string()
    .min(1, "Contact number is required")
    .regex(/^(09|\+639)\d{9}$/, "Enter a valid PH mobile number (e.g. 09123456789"),
});
...
const PROGRAMS = [
  { value: "regular", label: "Mainstream / Regular", desc: "Standard DepEd Basic Education Curriculum", color: "blue" },
  { value: "ste", label: "STE", desc: "Science, Technology & Engineering", color: "purple" },
  { value: "spfl", label: "SPFL", desc: "Special Program in Foreign Language", color: "teal" },
  { value: "open_high", label: "Open High School", desc: "Flexible learning for independent study", color: "amber" },
  { value: "als_shs", label: "ALS SHS", desc: "Alternative Learning System", color: "emerald" },
];

const PROGRAM_BADGES: Record<string, { bg: string; text: string; label: string }> = {
...
export function EnrollmentModule() {
  ...
  const [requirements, setRequirements] = useState<Record<string, boolean>>({
    psa_birth_cert: false, previous_grade_card: false, good_moral: false,
    id_photo: false, medical_clearance: false, parent_consent: false,
    transcript: false, lrn_verification: false,
  });
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      ...
      if (dropSearchRef.current && !dropSearchRef.current.contains(e.target as Node)) {
  ...
  const validateNewField = (field: string, value: string) => {
    const fieldSchema = z.object({
      ...
      guardian: z.string().min(1, "Guardian name is required").min(2, "Guardian name must be at least 2 characters"),
      contact: z.string().min(1, "Contact number is required").regex(/^(09|\+639)\d{9}$/, "Enter a valid PH mobile number (e.g. 09123456789)"),
      classifications: z.array(z.string()).optional(),
    });
    const result = fieldSchema.shape[field as keyof typeof fieldSchema.shape]?.safeParse(value);
    if (!result || result.success) {
      setNewErrors(prev => ({ ...prev, [field]: "" }));
    } else {
      const err = result.error as { issues: Array<{ message: string }> };
      setNewErrors(prev => ({ ...prev, [field]: err.issues[0]?.message || "" }));
    }
  };
  ...
  const handleDropSearch = async () => {
    if (!dropSearch.trim()) { setDropErrors({ search: "Enter an LRN or Student ID" }); return; }
    setDropErrors({});
    try {
  ...
  const handleConfirmNewEnrollment = async () => {
    ...
    try {
      ...
      if (sectionMatch) {
        ...
        setEnrolledSectionName(sectionMatch.name);
      } else {
  ...
  const handleConfirmReturning = async () => {
    ...
    try {
      ...
      await studentsApi.update(foundStudent.id, { grade_level: retGrade, status: "enrolled" });
      ...
      setEnrolledSectionName(sectionMatch?.name || "Pending Section");
      setEnrolledRet(true);
    } catch (err: any) {
  ...
  const handleConfirmDrop = async () => {
    ...
    try {
      ...
      const enrollment = allEnrollments.find(
        e => e.student_id === dropFound.id && e.school_year_id === selectedSYId && e.status === "enrolled"
      );
      if (enrollment) {
      ...
      <div className="space-y-6 max-w-5xl mx-auto">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="p-5 sm:p-6 flex items-center gap-4">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900">Enrollment Module</h2>
              <p className="text-gray-500 text-sm">Manage student enrollment, re-enrollment, and drop/transfer processing</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          >
            ...
            <div className="relative p-7 sm:p-8 flex flex-col items-center gap-4">
              ...
              <div className="text-center">
                <p className="font-bold text-gray-900 text-lg">Enroll New Student</p>
                <p className="text-gray-500 text-sm mt-1 leading-relaxed">Complete data entry for first-time enrollees with auto-ID generation</p>
                <div className="mt-4 flex flex-wrap justify-center gap-2">
                  {["Full data entry", "Auto-ID", "Section assignment"].map(t => (
                    <span key={t} className="bg-green-100 text-green-700 text-[11px] font-medium px-2.5 py-1 rounded-full">{t}</span>
                  ))}
                </div>
              </div>
              <div className="flex items-center gap-2 text-green-700 font-semibold text-sm group-hover:gap-3 transition-all duration-300">
                Start Enrollment <ChevronRight size={15} className="group-hover:translate-x-0.5 transition-transform" />
              </div>
            </div>
          </button>
          ...
          >
            ...
            <div className="relative p-7 sm:p-8 flex flex-col items-center gap-4">
              ...
              <div className="text-center">
                <p className="font-bold text-gray-900 text-lg">Enroll Returning Student</p>
                <p className="text-gray-500 text-sm mt-1 leading-relaxed">Auto-populate from existing records via LRN or name search</p>
                <div className="mt-4 flex flex-wrap justify-center gap-2">
                  {["LRN search", "Auto-populate", "Grade promotion"].map(t => (
                    <span key={t} className="bg-emerald-100 text-emerald-700 text-[11px] font-medium px-2.5 py-1 rounded-full">{t}</span>
                  ))}
                </div>
              </div>
              <div className="flex items-center gap-2 text-emerald-700 font-semibold text-sm group-hover:gap-3 transition-all duration-300">
                Search Student <ChevronRight size={15} className="group-hover:translate-x-0.5 transition-transform" />
              </div>
            </div>
          </button>
          ...
          >
            ...
            <div className="relative p-7 sm:p-8 flex flex-col items-center gap-4">
              ...
              <div className="text-center">
                <p className="font-bold text-gray-900 text-lg">Student Drop / Transfer</p>
                <p className="text-gray-500 text-sm mt-1 leading-relaxed">Process student dropout or school transfer with official reason documentation</p>
                <div className="mt-4 flex flex-wrap justify-center gap-2">
                  {["Dropout", "Transfer Out", "Transfer In", "Reason on record"].map(t => (
                    <span key={t} className="bg-red-100 text-red-600 text-[11px] font-medium px-2.5 py-1 rounded-full">{t}</span>
                  ))}
                </div>
              </div>
              <div className="flex items-center gap-2 text-red-600 font-semibold text-sm group-hover:gap-3 transition-all duration-300">
                Process Drop <ChevronRight size={15} className="group-hover:translate-x-0.5 transition-transform" />
              </div>
            </div>
          </button>
        </div>
  ...
  // ── NEW STUDENT FLOW ──────────────────────────────────────
  if (flow === "new") {
    const steps = ["Personal Details", "Grade & Program", "Requirements", "Student ID", "Preview & Confirm"];
    const fullName = [newData.firstName, newData.middleName, newData.lastName].filter(Boolean).join(" ");
    ...
    if (enrolledNew) {
      return (
        <div className="max-w-lg mx-auto text-center py-12 sm:py-16">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-1">Enrollment Successful!</h2>
          <p className="text-gray-500 text-sm mb-6">{fullName} has been successfully enrolled.</p>
          <div className="bg-white/80 backdrop-blur-sm border border-emerald-100 shadow-xl shadow-emerald-100/30 rounded-2xl p-6 mb-6 text-left space-y-3">
            <div className="flex items-center gap-3 pb-3 border-b border-emerald-100/60">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">Student ID: <span className="font-mono text-emerald-700 font-semibold">{newStudentID}</span></p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                <p className="text-[11px] text-gray-400 uppercase tracking-[0.04em] font-semibold">Grade Level</p>
                <p className="font-semibold text-gray-800">Grade {newGrade}</p>
              </div>
              <div>
                <p className="text-[11px] text-gray-400 uppercase tracking-[0.04em] font-semibold">Section</p>
                <p className="font-semibold text-emerald-700">{enrolledSectionName}</p>
              </div>
              <div>
                <p className="text-[11px] text-gray-400 uppercase tracking-[0.04em] font-semibold">LRN</p>
                <p className="font-mono text-xs text-gray-700">{newData.lrn}</p>
              </div>
              <div>
                <p className="text-[11px] text-gray-400 uppercase tracking-[0.04em] font-semibold">Program</p>
                <p>
                  <span className={`inline-block px-2 py-0.5 rounded-full text-[11px] font-semibold ${PROGRAM_BADGES[program]?.bg || "bg-blue-100"} ${PROGRAM_BADGES[program]?.text || "text-blue-700"}`}>
          ...
          <button onClick={resetAll} className="bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-700 hover:to-green-700 text-white px-8 py-3 rounded-xl font-semibold transition-all duration-200 shadow-lg shadow-emerald-200 hover:shadow-xl hover:shadow-emerald-300">
    ...
    return (
      <div className="max-w-2xl mx-auto space-y-5">
        <div className="flex items-center gap-4 bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5">
          <button onClick={resetAll} className="w-9 h-9 rounded-xl bg-gray-50 border border-gray-200 hover:bg-gray-100 flex items-center justify-center text-gray-500 transition-all flex-shrink-0">
            <ArrowLeft size={16} />
          </button>
          <div className="flex items-center gap-4 flex-1">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm">Complete all steps to finish enrollment</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sm:p-6">
          <div className="flex items-center justify-between relative">
            <div className="absolute top-5 left-0 right-0 h-1 bg-gray-100 z-0 rounded-full" style={{ left: "8%", right: "8%" }} />
            <div className="absolute top-5 left-0 right-0 h-1 z-0 rounded-full overflow-hidden" style={{ left: "8%", right: "8%" }}>
              <div
                className="h-full bg-gradient-to-r from-emerald-500 to-green-500 transition-all duration-500 rounded-full"
                style={{ width: `${((newStep - 1) / (steps.length - 1)) * 100}%` }}
              />
            </div>
            {steps.map((s, i) => {
              const stepNum = i + 1;
              ...
              return (
                <div key={s} className="flex flex-col items-center gap-2 z-10 flex-1">
                  ...
                  <p className={`text-[11px] font-semibold text-center leading-tight hidden sm:block transition-all duration-200 ${
          ...
          {newStep === 1 && (
            <div className="space-y-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 text-sm">Personal Details</h3>
                  <p className="text-gray-400 text-[11px]">Enter the student's basic personal information</p>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-[0.04em] mb-1.5">First Name *</label>
                  <input type="text" value={newData.firstName} onChange={e => updateNewField("firstName", e.target.value)}
                    ...
                    placeholder="Maria" />
                  {newErrors.firstName && <p className="text-red-500 text-xs mt-1 flex items-center gap-1"><AlertCircle size={10} />{newErrors.firstName}</p>}
                </div>
                <div>
                  <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-[0.04em] mb-1.5">Middle Name</label>
                  <input type="text" value={newData.middleName} onChange={e => updateNewField("middleName", e.target.value)}
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-[0.04em] mb-1.5">Last Name *</label>
                  <input type="text" value={newData.lastName} onChange={e => updateNewField("lastName", e.target.value)}
                    ...
                    placeholder="Santos" />
                  {newErrors.lastName && <p className="text-red-500 text-xs mt-1 flex items-center gap-1"><AlertCircle size={10} />{newErrors.lastName}</p>}
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-[0.04em] mb-1.5">Birthdate *</label>
                  <input type="date" value={newData.birthdate} onChange={e => updateNewField("birthdate", e.target.value)}
                  ...
                  {newErrors.birthdate && <p className="text-red-500 text-xs mt-1 flex items-center gap-1"><AlertCircle size={10} />{newErrors.birthdate}</p>}
                </div>
                <div>
                  <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-[0.04em] mb-1.5">Sex *</label>
                  <select value={newData.sex} onChange={e => updateNewField("sex", e.target.value)}
                  ...
                  {newErrors.sex && <p className="text-red-500 text-xs mt-1 flex items-center gap-1"><AlertCircle size={10} />{newErrors.sex}</p>}
                </div>
              </div>
              <div>
                <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-[0.04em] mb-1.5"><MapPin size={11} className="inline mr-1" />Complete Address *</label>
                <input type="text" value={newData.address} onChange={e => updateNewField("address", e.target.value)}
                ...
                {newErrors.address && <p className="text-red-500 text-xs mt-1 flex items-center gap-1"><AlertCircle size={10} />{newErrors.address}</p>}
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-[0.04em] mb-1.5">LRN (12 digits) *</label>
                  <input type="text" maxLength={12} value={newData.lrn} onChange={e => updateNewField("lrn", e.target.value)}
                  ...
                  {newErrors.lrn && <p className="text-red-500 text-xs mt-1 flex items-center gap-1"><AlertCircle size={10} />{newErrors.lrn}</p>}
                </div>
                <div>
                  <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-[0.04em] mb-1.5">Guardian Name *</label>
                  <input type="text" value={newData.guardian} onChange={e => updateNewField("guardian", e.target.value)}
                  ...
                  {newErrors.guardian && <p className="text-red-500 text-xs mt-1 flex items-center gap-1"><AlertCircle size={10} />{newErrors.guardian}</p>}
                </div>
                <div>
                  <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-[0.04em] mb-1.5"><Phone size={11} className="inline mr-1" />Contact Number *</label>
                  <input type="text" value={newData.contact} onChange={e => updateNewField("contact", e.target.value)}
                  ...
                  {newErrors.contact && <p className="text-red-500 text-xs mt-1 flex items-center gap-1"><AlertCircle size={10} />{newErrors.contact}</p>}
                </div>
              </div>
              <div>
                <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-[0.04em] mb-2">Classification (check all that apply)</label>
                <div className="flex flex-wrap gap-3">
                  {["4Ps Beneficiary", "PWD", "Transferee", "Non-Reader"].map(cls => (
                    <label key={cls} className={`flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl border-2 cursor-pointer transition-all duration-200 ${newData.classifications.includes(cls) ? "border-emerald-400 bg-emerald-50 text-emerald-700 shadow-sm" : "border-gray-100 bg-gray-50/50 text-gray-600 hover:border-gray-200 hover:bg-gray-50"}`}>
                      <input type="checkbox" checked={newData.classifications.includes(cls)} onChange={() => toggleClassification(cls)} className="accent-emerald-600 w-4 h-4" />
          ...
          {newStep === 2 && (
            <div className="space-y-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 text-sm">Grade Level & Program</h3>
                  <p className="text-gray-400 text-[11px]">Select the grade level and curriculum program</p>
                </div>
              </div>
              <div>
                <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-[0.04em] mb-2.5">Grade Level *</label>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {GRADE_LEVELS.map(g => (
                    ...
                    >
                      <p className={`font-bold text-lg ${newGrade === g ? "text-emerald-700" : "text-gray-700"}`}>Grade {g}</p>
                      <p className={`text-[11px] mt-0.5 font-medium ${newGrade === g ? "text-emerald-500" : "text-gray-400"}`}>{g <= 10 ? "Junior High" : "Senior High"}</p>
                      {newGrade === g && (
              ...
              {/* Program selector */}
              <div>
                <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-[0.04em] mb-2.5">Curriculum Program *</label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {PROGRAMS.map(p => {
                    const active = program === p.value;
                    const borderCls = active ? "border-emerald-400 bg-emerald-50" : "border-gray-100 bg-gray-50/50 hover:border-gray-200 hover:bg-gray-50";
                    const dotCls = active ? "border-emerald-500" : "border-gray-300";
                    ...
                    >
                      <div className={`w-5 h-5 rounded-full border-2 mt-0.5 flex items-center justify-center flex-shrink-0 transition-colors ${dotCls}`}>
                        {active && <div className={`w-2.5 h-2.5 rounded-full ${innerDotCls}`} />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className={`font-semibold text-sm ${active ? "text-emerald-700" : "text-gray-800"}`}>{p.label}</p>
          ...
          {newStep === 3 && (
            <div className="space-y-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 text-sm">Enrollment Requirements</h3>
                  <p className="text-gray-400 text-[11px]">Check off the requirements the student has submitted. These can be submitted later.</p>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {REQUIREMENTS_LIST.map(r => (
                  <button
                    key={r.key}
                    type="button"
                    onClick={() => setRequirements(prev => ({ ...prev, [r.key]: !prev[r.key] }))}
                    className={`flex items-start gap-3.5 p-4 rounded-xl border-2 cursor-pointer transition-all duration-200 text-left w-full ${
                      requirements[r.key] ? "border-emerald-400 bg-emerald-50/80 shadow-sm" : "border-gray-100 bg-gray-50/50 hover:border-gray-200 hover:bg-gray-50"
                    }`}
                  >
                    <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center mt-0.5 flex-shrink-0 transition-all duration-200 ${
                      requirements[r.key] ? "bg-emerald-500 border-emerald-500 shadow-sm shadow-emerald-200" : "border-gray-300 bg-white"
                    }`}>
                      {requirements[r.key] && <Check size={12} className="text-white" strokeWidth={3} />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <span className={`text-sm block ${requirements[r.key] ? "text-emerald-800 font-medium" : "text-gray-600"}`}>{r.label}</span>
                    </div>
                  </button>
                ))}
              </div>
              <div className="bg-emerald-50/50 border border-emerald-100 rounded-xl px-4 py-3 flex items-center gap-3">
                ...
                <p className="text-xs text-emerald-700 font-medium">
                  {Object.values(requirements).filter(Boolean).length} of {REQUIREMENTS_LIST.length} requirements checked
                </p>
              </div>
            </div>
          )}
            ...
            <div className="space-y-5 text-center">
              <div className="flex items-center gap-3 justify-center">
                ...
                <div className="text-left">
                  <h3 className="font-bold text-gray-900 text-sm">Auto-Generated Student ID</h3>
                  <p className="text-gray-400 text-[11px]">System-generated unique identifier for this student</p>
                </div>
              </div>
              <div className="bg-gradient-to-br from-emerald-50 to-green-50 border-2 border-emerald-200 rounded-2xl p-8 shadow-inner">
                ...
                <p className="text-gray-500 text-sm mb-2 tracking-[0.04em]">System Generated Student ID</p>
                <p className="text-4xl sm:text-5xl font-black text-emerald-700 font-mono tracking-[0.08em]">{newStudentID}</p>
                <div className="mt-6 flex flex-wrap justify-center gap-5 text-xs text-gray-500">
                  <span className="px-3 py-1.5 bg-white/70 rounded-lg border border-emerald-100"><span className="font-bold text-emerald-700">{newStudentID.split("-")[0]}</span> — School Year</span>
                  <span className="px-3 py-1.5 bg-white/70 rounded-lg border border-emerald-100"><span className="font-bold text-emerald-700">{newStudentID.split("-")[1]}</span> — Grade Level</span>
                  <span className="px-3 py-1.5 bg-white/70 rounded-lg border border-emerald-100"><span className="font-bold text-emerald-700">{newStudentID.split("-")[2]}</span> — Sequence No.</span>
                </div>
              </div>
              <p className="text-gray-400 text-xs">This ID is unique and will be used for all records of this student throughout their academic journey.</p>
            </div>
          )}
          ...
          {newStep === 5 && (
            <div className="space-y-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 text-sm">Enrollment Preview</h3>
                  <p className="text-gray-400 text-[11px]">Review all details before confirming enrollment</p>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-gray-50/70 border border-gray-100 rounded-xl p-5 space-y-3">
                  <p className="text-[11px] font-bold text-gray-500 uppercase tracking-[0.06em] flex items-center gap-2"><User size={13} /> Student Information</p>
                  {[
                    ["Full Name", fullName || "—"],
                    ...
                    <div key={k} className="flex justify-between text-sm border-b border-gray-100/80 pb-2 last:border-0 last:pb-0">
                ...
                <div className="bg-gray-50/70 border border-gray-100 rounded-xl p-5 space-y-3">
                  <p className="text-[11px] font-bold text-gray-500 uppercase tracking-[0.06em] flex items-center gap-2"><MapPin size={13} /> Contact & Classification</p>
                  {[
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className={`font-medium ${PROGRAM_BADGES[program]?.text || "text-blue-700"}`}>{PROGRAM_BADGES[program]?.label || "Regular"}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50/70 border border-gray-100 rounded-xl p-5">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-[11px] font-bold text-gray-500 uppercase tracking-[0.06em] flex items-center gap-2"><FileText size={13} /> Requirements Submitted</p>
                  <span className="text-xs font-semibold text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-100">
                    {Object.values(requirements).filter(Boolean).length}/{REQUIREMENTS_LIST.length}
                  </span>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {REQUIREMENTS_LIST.map(r => (
                    <span key={r.key} className={`text-[11px] px-2.5 py-1 rounded-full font-medium ${
                      requirements[r.key] ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-400"
                    }`}>
                      {requirements[r.key] ? "✓" : "○"} {r.label.replace(/\(.*\)/, "").trim()}
                    </span>
                  ))}
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4">
                ...
                <div>
                  <p className="font-semibold text-amber-800 text-sm">Section Status: Pending Section</p>
                  <p className="text-amber-600 text-xs mt-0.5 leading-relaxed">New students without previous grades are temporarily assigned to Pending Section until their average is computed. The system will automatically move them to the appropriate section once grades are uploaded and computed.</p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="flex gap-3">
          ...
          {newStep < 5 ? (
            ...
            >
              {newStep === 4 ? "Continue to Preview" : "Next Step"} <ChevronRight size={15} />
            </button>
          ) : (
            <button
    ...
    if (enrolledRet && foundStudent) {
      return (
        <div className="max-w-lg mx-auto text-center py-12 sm:py-16">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-1">Re-enrollment Successful!</h2>
          <p className="text-gray-500 text-sm mb-6">{foundStudent.name} has been enrolled for Grade {retGrade}.</p>
          <div className="bg-white/80 backdrop-blur-sm border border-emerald-100 shadow-xl shadow-emerald-100/30 rounded-2xl p-6 mb-6 text-left space-y-3">
            <div className="flex items-center gap-3 pb-3 border-b border-emerald-100/60">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-green-600 shadow-md flex items-center justify-center flex-shrink-0">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">Student ID: <span className="font-mono text-emerald-700 font-semibold">{foundStudent.student_id}</span></p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-y-3 gap-x-4 text-sm">
              <div>
                <p className="text-[11px] text-gray-400 uppercase tracking-[0.04em] font-semibold">Previous Grade</p>
                <p className="font-semibold text-gray-700">Grade {foundStudent.grade_level}</p>
              </div>
              <div>
                <p className="text-[11px] text-gray-400 uppercase tracking-[0.04em] font-semibold">New Grade</p>
                <p className="font-semibold text-emerald-700">Grade {retGrade}</p>
              </div>
              <div>
                <p className="text-[11px] text-gray-400 uppercase tracking-[0.04em] font-semibold">Assigned Section</p>
                <p className="font-semibold text-emerald-700">{enrolledSectionName}</p>
              </div>
              <div>
                <p className="text-[11px] text-gray-400 uppercase tracking-[0.04em] font-semibold">Program</p>
      ...
      <div className="max-w-2xl mx-auto space-y-5">
        <div className="flex items-center gap-4 bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5">
          ...
          <div className="flex items-center gap-4 flex-1">
            ...
            <div>
              <h2 className="font-bold text-gray-900">Enroll Returning Student</h2>
              <p className="text-gray-400 text-sm">Search by LRN or Student ID to auto-populate student records</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <div className="flex items-center justify-between relative">
            ...
            {["Search LRN/ID", "Student Details", "Grade & Program", "Confirm"].map((s, i) => {
              const stepNum = i + 1;
          ...
          {retStep === 1 && (
            <div className="space-y-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 text-sm">Search Student Record</h3>
                  <p className="text-gray-400 text-[11px]">Enter the student's LRN or Student ID to retrieve their record</p>
                </div>
              </div>
              <div className="flex gap-3" ref={searchRef}>
                <div className="relative flex-1">
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-400">
                              <span className="font-mono">{s.lrn}</span> · ID: {s.student_id} · Grade {s.grade_level}
                            </p>
                          </div>
                  ...
                  {retErrors.query && <p className="text-red-500 text-xs mt-1 flex items-center gap-1"><AlertCircle size={10} />{retErrors.query}</p>}
                </div>
                <button onClick={() => { setShowSuggestions(false); handleSearch(); }} className="bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-700 hover:to-green-700 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 shadow-md shadow-emerald-200">
                  Search
                </button>
              </div>
              {notFound && (
                <div className="flex items-center gap-2.5 text-red-600 bg-red-50/80 border border-red-200 rounded-xl px-4 py-3 text-sm">
                  ...
                  No student record found for "<strong>{searchQuery}</strong>". Please check the LRN or Student ID.
                </div>
              )}
              {allStudents.length > 0 && !showSuggestions && !foundStudent && (
                <div className="bg-gray-50/70 border border-gray-100 rounded-xl p-4">
                  <p className="text-[11px] text-gray-500 font-semibold uppercase tracking-[0.04em] mb-2.5">Quick access</p>
                  <div className="flex flex-wrap gap-2">
                    {allStudents.slice(0, 4).map(s => (
                      <button key={s.lrn} onClick={() => { setSearchQuery(s.lrn); setNotFound(false); setShowSuggestions(true); }} className="text-xs bg-white border border-gray-200 hover:border-emerald-300 text-gray-600 hover:text-emerald-700 hover:bg-emerald-50 px-3 py-1.5 rounded-lg transition-all">
                        {s.lrn} — {s.name}
                      </button>
                    ))}
                  </div>
          ...
          {retStep === 2 && foundStudent && (
            <div className="space-y-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 text-sm">Student Record Found</h3>
                  <p className="text-gray-400 text-[11px]">Details auto-populated from existing record</p>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-emerald-50/50 border border-emerald-100 rounded-xl p-5 space-y-3">
                  <p className="text-[11px] font-bold text-emerald-700 uppercase tracking-[0.06em] flex items-center gap-2"><User size={13} /> Student Information</p>
                  {[
                    ...
                    <div key={k} className="flex justify-between text-sm border-b border-emerald-100/50 pb-2 last:border-0 last:pb-0">
                      <span className="text-gray-500">{k}:</span>
                ...
                <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-5 space-y-3">
                  <p className="text-[11px] font-bold text-blue-700 uppercase tracking-[0.06em] flex items-center gap-2"><FileText size={13} /> Previous Academic Record</p>
                  {[
                    ["Previous Grade", `Grade ${foundStudent.grade_level}`],
                    ["Guardian", foundStudent.guardian || "—"],
                    ["Contact", foundStudent.contact || "—"],
                  ].map(([k, v]) => (
                    <div key={k} className="flex justify-between text-sm border-b border-blue-100/50 pb-2 last:border-0 last:pb-0">
                      <span className="text-gray-500">{k}:</span>
          ...
          {retStep === 3 && (
            <div className="space-y-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 text-sm">Grade Level & Program</h3>
                  <p className="text-gray-400 text-[11px]">Select the new grade level and curriculum program</p>
                </div>
              </div>
              <div>
                <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-[0.04em] mb-2.5">Grade Level *</label>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {GRADE_LEVELS.map(g => (
                    ...
                    >
                      <p className={`font-bold text-lg ${retGrade === g ? "text-emerald-700" : "text-gray-700"}`}>Grade {g}</p>
                      <p className={`text-[11px] mt-0.5 font-medium ${retGrade === g ? "text-emerald-500" : "text-gray-400"}`}>{g <= 10 ? "Junior High" : "Senior High"}</p>
                      {retGrade === g && (
                        <div className="mt-2 w-5 h-5 rounded-full bg-emerald-500 text-white flex items-center justify-center mx-auto shadow-sm">
                          <Check size={11} strokeWidth={3} />
                        </div>
          ...
          {retStep === 4 && foundStudent && (
            <div className="space-y-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 text-sm">Confirm Re-Enrollment</h3>
                  <p className="text-gray-400 text-[11px]">Please review the details before confirming</p>
                </div>
              </div>
              <div className="bg-gray-50/70 border border-gray-100 rounded-xl p-5 space-y-3">
                {[
                  ...
                  <div key={k} className="flex justify-between text-sm border-b border-gray-100/80 pb-2.5 last:border-0 last:pb-0">
                    <span className="text-gray-500">{k}:</span>
                    <span className={`font-medium text-right max-w-[55%] ${k.includes("Section") ? "text-emerald-700" : k === "New Grade Level" ? "text-emerald-700" : "text-gray-800"}`}>{v}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="flex gap-3">
          {retStep > 1 && (
            <button onClick={() => setRetStep((retStep - 1) as RetStep)} className="flex-1 border border-gray-200 hover:bg-gray-50 text-gray-600 rounded-xl text-sm font-semibold py-2.5 transition-all duration-200">
              ← Back
            </button>
          )}
          {retStep < 4 ? (
            ...
            <button onClick={handleConfirmReturning} className="flex-1 bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-700 hover:to-green-700 text-white py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 shadow-md shadow-emerald-200 hover:shadow-lg hover:shadow-emerald-300 flex items-center justify-center gap-2">
    ...
    if (dropDone && dropFound) {
      return (
        <div className="max-w-lg mx-auto text-center py-12 sm:py-16">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-1">{actionLabel} Processed</h2>
          <p className="text-gray-500 text-sm mb-6">{dropFound.name}'s record has been updated with {actionLabel} status.</p>
          <div className="bg-white/80 backdrop-blur-sm border border-red-100 shadow-xl shadow-red-100/30 rounded-2xl p-6 mb-6 text-left space-y-3">
            <div className="flex items-center gap-3 pb-3 border-b border-red-100/60">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">Student ID: <span className="font-mono text-gray-700">{dropFound.student_id}</span></p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-y-3 gap-x-4 text-sm">
              <div>
                <p className="text-[11px] text-gray-400 uppercase tracking-[0.04em] font-semibold">Action</p>
                <p className="font-semibold text-red-700">{actionLabel}</p>
              </div>
              <div>
                <p className="text-[11px] text-gray-400 uppercase tracking-[0.04em] font-semibold">Grade</p>
                <p className="font-semibold text-gray-700">Grade {dropFound.grade_level}</p>
              </div>
              <div className="col-span-2">
                <p className="text-[11px] text-gray-400 uppercase tracking-[0.04em] font-semibold">Reason</p>
                <p className="font-medium text-gray-700">{dropReason}</p>
              </div>
              <div className="col-span-2">
                <p className="text-[11px] text-gray-400 uppercase tracking-[0.04em] font-semibold">Date Processed</p>
                <p className="font-medium text-gray-700">{new Date().toLocaleDateString("en-PH", {year: "numeric", month: "long", day: "numeric"})}</p>
              </div>
            </div>
          </div>
          <button onClick={resetAll} className="bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-700 hover:to-orange-700 text-white px-8 py-3 rounded-xl font-semibold transition-all duration-200 shadow-lg shadow-red-200 hover:shadow-xl hover:shadow-red-300">
            Process Another
          </button>
        </div>
      );
      ...
      <div className="max-w-2xl mx-auto space-y-5">
        <div className="flex items-center gap-4 bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5">
          ...
          <div className="flex items-center gap-4 flex-1">
            ...
            <div>
              <h2 className="font-bold text-gray-900">Student Drop / Transfer</h2>
              <p className="text-gray-400 text-sm">Search for a student, select action reason, and confirm</p>
            </div>
          </div>
        </div>
            ...
            {["Search Student", "Select Reason", "Confirm"].map((s, i) => {
              ...
              return (
                <div key={s} className="flex flex-col items-center gap-2 z-10 flex-1">
                  ...
                  <p className={`text-[11px] font-semibold text-center leading-tight hidden sm:block transition-all duration-200 ${
                    active ? "text-red-700" : done ? "text-red-500" : "text-gray-400"
                  }`}>{s}</p>
                </div>
              );
            })}
          ...
          {dropStep === 1 && (
            <div className="space-y-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 text-sm">Search Student Record</h3>
                  <p className="text-gray-400 text-[11px]">Enter the student's LRN or Student ID to locate their record</p>
                </div>
              </div>
              <div className="flex gap-3" ref={dropSearchRef}>
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    onChange={e => { setDropSearch(e.target.value); setDropNotFound(false); setDropErrors({}); setDropShowSuggestions(e.target.value.trim().length > 0); }}
                    onKeyDown={e => {
                      if (e.key === "Enter") { setDropShowSuggestions(false); handleDropSearch(); }
                    }}
                    onFocus={() => { if (dropSearch.trim()) setDropShowSuggestions(true); }}
                  ...
                  {dropErrors.search && <p className="text-red-500 text-xs mt-1 flex items-center gap-1"><AlertCircle size={10} />{dropErrors.search}</p>}
                </div>
                <button
                  onClick={() => { setDropShowSuggestions(false); handleDropSearch(); }}
              ...
              {dropNotFound && (
                <div className="flex items-center gap-2.5 text-red-600 bg-red-50/80 border border-red-200 rounded-xl px-4 py-3 text-sm">
                  <AlertCircle size={16} className="flex-shrink-0" /> No record found for "<strong>{dropSearch}</strong>". Please check the LRN or Student ID.
                </div>
              )}
              {allStudents.length > 0 && !dropFound && !dropShowSuggestions && (
                <div className="bg-gray-50/70 border border-gray-100 rounded-xl p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {allStudents.slice(0, 4).map(s => (
                      <button key={s.lrn} onClick={() => { setDropSearch(s.lrn); setDropNotFound(false); setDropShowSuggestions(true); }} className="text-xs bg-white border border-gray-200 hover:border-red-300 text-gray-600 hover:text-red-700 hover:bg-red-50 px-3 py-1.5 rounded-lg transition-all">
                        {s.lrn} — {s.name}
                      </button>
                    ))}
                  </div>
          ...
          {dropStep === 2 && dropFound && (
            <div className="space-y-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 text-sm">Student Found — Select Action & Reason</h3>
                  <p className="text-gray-400 text-[11px]">Choose the reason for this drop or transfer</p>
                </div>
              </div>
              {/* Student info strip */}
              ...
              <div>
                <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-[0.04em] mb-2.5">Action / Reason *</label>
                <div className="space-y-2">
                  {DROP_REASONS.map(r => (
                    ...
                    >
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${dropReason === r ? "border-red-500" : "border-gray-300"}`}>
                        {dropReason === r && <div className="w-2.5 h-2.5 rounded-full bg-red-500" />}
                      </div>
              ...
              <div>
                <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-[0.04em] mb-1.5">Additional Remarks (optional)</label>
                <textarea
                  value={dropRemarks}
          ...
          {dropStep === 3 && dropFound && (
            <div className="space-y-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 text-sm">Confirm {actionLabel}</h3>
                  <p className="text-gray-400 text-[11px]">Please review the details before confirming this action</p>
                </div>
              </div>
              <div className="bg-red-50/70 border border-red-100 rounded-xl p-5 space-y-3">
                {[
                  ...
                  ["Date Processed", new Date().toLocaleDateString("en-PH", {year: "numeric", month: "long", day: "numeric"})],
                ].map(([k, v]) => (
                  <div key={k} className="flex justify-between text-sm border-b border-red-100/60 pb-2.5 last:border-0 last:pb-0">
                    <span className="text-gray-500">{k}:</span>
              ...
              <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4">
                ...
                <div>
                  ...
                  <p className="text-amber-600 text-xs mt-0.5 leading-relaxed">This action will update the student's enrollment status in the system. The student's academic records will be preserved for reference and SF10 generation.</p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="flex gap-3">
          {dropStep > 1 && (
            <button onClick={() => setDropStep((dropStep - 1) as DropStep)} className="flex-1 border border-gray-200 hover:bg-gray-50 text-gray-600 rounded-xl text-sm font-semibold py-2.5 transition-all duration-200">
              ← Back
            </button>
          )}
          {dropStep < 3 ? (
            <button
              onClick={() => {
                if (dropStep === 2) {
                  const result = dropReasonSchema.safeParse({ reason: dropReason });
                  if (!result.success) {
                    showToast("error", "Please select a reason before proceeding.");
                    return;
                  }
                }
