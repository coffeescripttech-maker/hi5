...
export function UploadGrades() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        showToast(
          'error',
          err.detail?.error || err.message || 'Failed to load data'
        );
      } finally {
  ...
  // Subjects available for the selected section's grade level — NOT filtered by
  // assignments, so the template download always works (it's only a blank roster).
  // Grade importing stays scoped: handleFileChosen rejects subjects the teacher is
  // not assigned to, matching the backend's grade-encoding scope.
  ...
  useEffect(() => {
    if (
      selectedSubjectId &&
      !sectionSubjects.some(s => s.id === selectedSubjectId)
    ) {
  ...
  const hasSelection =
    !!selectedSectionId && !!selectedSubjectId && !!selectedYearId;
  ...
  const visibleRows = useMemo(() => {
    ...
    return previewRows.filter(r => {
      ...
      const matchSearch =
        !q || r.lrn.includes(q) || (r.name || '').toLowerCase().includes(q);
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      ...
      showToast(
        'error',
        err.detail?.error || err.message || 'Template download failed'
      );
    } finally {
  ...
  const handleFileChosen = async (file: File) => {
    if (!hasSelection) {
      showToast(
        'error',
        'Please select Section, Subject, and School Year first.'
      );
      return;
    }

    if (!assignedSubjectIds.has(selectedSubjectId as number)) {
      showToast(
        'error',
        'You may only upload grades for subjects assigned to you.'
      );
      return;
    }
    ...
    <div className="space-y-5">
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
        ...
        <div className="p-5 sm:p-6">
          <div className="flex items-start gap-4 mb-4">
            ...
            <div>
              <h2 className="font-bold text-gray-900 text-lg tracking-[-0.02em]">
                Upload Past Grades
              </h2>
              <p className="text-gray-500 text-sm">
                Pick a section, subject, and quarter, then download the roster
                template and fill in grades.
              </p>
            </div>
          </div>
            ...
            <div>
              ...
              {sectionSubjects.length === 0 && (
                <p className="text-[11px] text-blue-700 bg-blue-50 border border-blue-200 rounded-lg px-2.5 py-1.5 mt-1.5">
                  No subjects are set up for this section's grade level yet.
                </p>
              )}
            </div>
            <div>
              <label className="flex items-center gap-1.5 text-xs font-semibold text-gray-600 mb-1">
                <CalendarDays size={13} className="text-emerald-600" /> School
                Year
              </label>
              <select
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 border-t border-gray-100 pt-4">
            <button
              ...
              title={
                !hasSelection
                  ? 'Select section, subject, and school year first'
                  : undefined
              }
            ...
            <div className="text-gray-400 text-xs flex items-center gap-1">
              ...
              Template format: LRN | Student Name | Grade — one file per subject
              &amp; quarter
            </div>
          </div>
        </div>
              ...
              <React.Fragment key={step.label}>
                <div className="flex items-center gap-2 flex-1 last:flex-none">
                  ...
                  <div className="hidden md:block leading-tight">
                    <p
                      className={`text-xs font-semibold ${active ? 'text-emerald-700' : 'text-gray-700'}`}>
                      {step.label}
                    </p>
                ...
                {i < STEPS.length - 1 && (
                  <div
                    className={`h-0.5 flex-1 mx-2 mb-5 md:mb-0 ${complete ? 'bg-emerald-500' : 'bg-gray-200'}`}
                  />
                )}
              </React.Fragment>
            );
      ...
      {stage === 'idle' && (
        <div
          ...
          />
          <div
            className={`w-16 h-16 rounded-2xl mx-auto mb-4 flex items-center justify-center transition-colors ${dragActive ? 'bg-emerald-600' : 'bg-emerald-100'}`}>
            <Upload
              size={28}
              className={dragActive ? 'text-white' : 'text-emerald-600'}
            />
          </div>
          ...
          <p className="text-gray-400 text-sm mb-4">
            or click to browse · .xlsx, .xls formats
          </p>
          ...
          {loading && (
            <p className="text-xs text-gray-400 mt-3">
              Loading sections, subjects, and school years…
            </p>
          )}
        </div>
      )}
      ...
      {stage === 'uploading' && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 text-center">
          ...
          <p className="text-gray-400 text-sm mb-4">
            Validating data format and checking for errors...
          </p>
      ...
      {stage === 'preview' && (
        <div className="space-y-4">
            ...
            <div className="bg-red-50 border border-red-200 rounded-2xl p-4">
              <div className="flex items-center gap-2 text-red-700 mb-2">
                ...
                <span className="font-semibold text-sm">
                  {invalidRows.length} Error(s) Found – These rows will be
                  skipped
                </span>
              </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between gap-3 flex-wrap">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-gray-800 truncate max-w-[260px]">
                    {fileName}
                  </p>
                  <p className="text-xs text-gray-400">
                    Parsed and validated — green rows will be imported
                  </p>
                </div>
              </div>
            ...
            <div className="px-5 py-3 border-b border-gray-100 flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                {(
                  ['all', 'valid', 'skipped', 'invalid'] as PreviewFilter[]
                ).map(f => (
                  <button
                    key={f}
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {visibleRows.map(r => (
                          <tr
                            ...
                            <td className="px-5 py-2 text-gray-800 font-medium">
                              {r.grade ?? '—'}
                            </td>
                            <td className="px-5 py-2">
                              ...
                              {r.status === 'skipped' && (
                                <span className="inline-flex items-center gap-1 text-yellow-600 text-xs font-semibold">
                                  <SkipForward size={13} /> Blank grade
                                </span>
                              )}
                              {r.status === 'invalid' && (
                                <span
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-gray-400 mt-0.5">
                              <span className="font-mono">{r.lrn}</span> · Row{' '}
                              {r.row}
                            </p>
                          ...
                          <div className="flex-shrink-0">
                            {r.status === 'valid' && (
                              <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-emerald-100 text-emerald-700 text-[11px] font-semibold border border-emerald-200">
                                <CheckCircle size={11} /> Ready
                              </span>
                            )}
                            {r.status === 'skipped' && (
                              <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-yellow-100 text-yellow-700 text-[11px] font-semibold border border-yellow-200">
                                <SkipForward size={11} /> Blank
                              </span>
                            )}
                            {r.status === 'invalid' && (
                              <span
                                className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-red-100 text-red-700 text-[11px] font-semibold border border-red-200"
                                title={r.error}>
                                Invalid
                              </span>
                            )}
            ...
            <div className="px-5 py-3 bg-gray-50 border-t border-gray-100">
              <p className="text-xs text-gray-400">
                ...
                {previewRows.length} row(s).{' '}
                <strong className="text-emerald-600">Ready</strong> rows will be
                imported · <strong className="text-yellow-600">blank</strong>{' '}
                grades are skipped ·{' '}
                <strong className="text-red-600">errors</strong> are blocked.
                Already-locked grades are skipped during import.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              onClick={reset}
              className="flex items-center justify-center gap-2 border border-gray-300 text-gray-700 px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-gray-50 transition">
              <X size={14} /> Cancel
            </button>
            <button
      ...
      {stage === 'importing' && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 text-center">
          ...
          <p className="text-gray-400 text-sm mb-4">
            Writing validated grades and skipping locked entries
          </p>
          <div className="w-full bg-gray-100 rounded-full h-2 max-w-sm mx-auto">
            <div
              className="bg-emerald-600 h-2 rounded-full transition-all"
      ...
      {stage === 'done' && (
        <div className="space-y-5">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              onClick={reset}
              className="flex items-center justify-center gap-2 bg-emerald-700 hover:bg-emerald-800 text-white px-6 py-2.5 rounded-lg font-medium transition text-sm shadow-sm">
              <RefreshCw size={14} /> Upload Another File
            </button>
            <a
              href="/teacher/documents"
              className="flex items-center justify-center gap-2 border border-gray-300 text-gray-700 px-6 py-2.5 rounded-lg font-medium transition hover:bg-gray-50 text-sm">
