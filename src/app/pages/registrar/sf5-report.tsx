...
const LEAF_COLUMNS: Leaf[] = [
  { key: 'lrn', width: '35px' },
  { key: 'name', width: '90px' },
  { key: 'average', width: '75px' },
  { key: 'action', width: '85px' },
  { key: 'incomplete_completed', width: '140px' },
  { key: 'incomplete_current', width: '140px' }
];
...
/* ---------------------------------------------------------------- */
/* MiniCountTable â€” reusable summary table with input cells          */
/* ---------------------------------------------------------------- */
...
export function SF5Report() {
  ...
  useEffect(() => {
    ...
    if (enrolled.length > 0) {
      ...
      Promise.all([
        ...
        prevSyId
          ? gradesApi.list({ school_year_id: prevSyId })
          : Promise.resolve<GradeRow[]>([])
      ])
        .then(([cur, prev]) => {
          if (cancelled) return;
          ...
          > = {};
          const summary = SUMMARY_STATUS.map(() => ({
            male: 0,
            female: 0,
            total: 0
          }));
          const progress = PROGRESS_LEVELS.map(() => ({
            male: 0,
            female: 0,
            total: 0
          }));
          ...
          enrolled.forEach(s => {
            ...
            const failedNow = [...finals.values()]
              .filter(f => f.avg < 75)
              .map(f => f.name);
            const genAvg = finals.size
              ? mean([...finals.values()].map(f => f.avg))
              : null;
            ...
            if (genAvg != null) {
              ...
              else if (failedNow.length <= 2)
                action = '*Conditionally Promoted';
            ...
            if (genAvg != null) {
              ...
              const si =
                failedNow.length === 0 ? 0 : failedNow.length <= 2 ? 1 : 2;
              summary[si].total++;
  ...
  const totalRow = (label: string, count: number) => (
    <tr className="bg-gray-50/80">
      ...
      <td className="border border-black bg-gray-200 p-0" />
      <td className="border border-black bg-gray-200 p-0">
        <span className="flex h-full items-center justify-center text-[11px] font-bold">
          {count}
        </span>
      </td>
    </tr>
    ...
    <div className="space-y-5 pb-10">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-lg font-bold text-gray-900 tracking-[-0.02em]">
                  School Form 5 (SF5) â€” Report on Promotion &amp; Level of
      ...
      <DocumentViewer>
        <div
          ...
          <div className="sf1-page sf5-page" style={{ minWidth: '840px' }}>
            {/* ---------- Title block ---------- */}
            <div className="mb-4 flex items-center justify-between gap-6">
              ...
              <div className="px-4 text-center">
                ...
                <p className="text-[9px] italic text-black/80">
                  (This replaces Form 5, Report on Promotion and Level of
                  Progress &amp; Achievement)
                </p>
              </div>
              <img
                ...
                <table
                  ...
                  <thead>
                    ...
                    <tr>
                      <th
                        style={{ width: '140px' }}
                        className="border border-black px-1 py-1 align-middle font-semibold text-blue-800">
                        Incomplete Learning Area/s from previous school year/s
                        that <u>had been completed</u> as of end of current
                        School Year
                      </th>
                      <th
                        style={{ width: '140px' }}
