...
import {
  ...
  GraduationCap, Layers, Search, Filter, BarChart3
} from "lucide-react";
...
export function SectionAssignment() {
  ...
  const SectionPicker = ({
    ...
    return (
      <select
        className="text-xs border rounded-lg px-2 py-1.5 bg-white w-44"
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            <h2 className="font-bold text-gray-800 text-lg">Section Assignment</h2>
          ...
          <button
            onClick={loadQueue}
            disabled={loading}
            className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-600 bg-gray-50 hover:bg-gray-100 rounded-lg border border-gray-200 transition-colors disabled:opacity-50"
          >
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
          ...
          <div className="flex flex-wrap gap-3 mb-4">
            {gradeCounts.length > 0 ? gradeCounts.map(gc => (
              ...
              >
                ...
                <span className="text-blue-400 text-xs">{gc.count === 1 ? "student" : "students"}</span>
              </div>
            )) : (
              <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-50 text-gray-400 rounded-lg text-sm">
                <Users size={15} />
                <span>No students in queue</span>
              </div>
            )}
            {queue.length > 0 && (
              <div className="flex items-center gap-2 px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-lg text-sm font-semibold">
                Total: {queue.length}
              </div>
            )}
          </div>
        ...
        {loading ? (
          <div className="text-center py-8 text-gray-400 border border-dashed border-gray-200 rounded-lg">
            <Loader2 size={28} className="mx-auto mb-2 animate-spin text-blue-400" />
            ...
            <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    <th className="text-left px-3 py-2 text-gray-500 font-semibold text-[10px] uppercase tracking-wider">#</th>
                    <th className="text-left px-3 py-2 text-gray-500 font-semibold text-[10px] uppercase tracking-wider">Student</th>
                    <th className="text-left px-3 py-2 text-gray-500 font-semibold text-[10px] uppercase tracking-wider">LRN</th>
                    <th className="text-left px-3 py-2 text-gray-500 font-semibold text-[10px] uppercase tracking-wider">Grade</th>
                    <th className="text-left px-3 py-2 text-gray-500 font-semibold text-[10px] uppercase tracking-wider">Program</th>
                    <th className="text-left px-3 py-2 text-gray-500 font-semibold text-[10px] uppercase tracking-wider">GA</th>
                    <th className="text-left px-3 py-2 text-gray-500 font-semibold text-[10px] uppercase tracking-wider">Enrolled By</th>
                    <th className="text-left px-3 py-2 text-gray-500 font-semibold text-[10px] uppercase tracking-wider">Date</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {queue.map((s, idx) => (
                    <tr key={s.enrollment_id} className="hover:bg-blue-50/30 transition">
                      <td className="px-3 py-2 text-gray-400 text-xs">{idx + 1}</td>
                      <td className="px-3 py-2">
                        <p className="font-medium text-gray-800 text-sm">{s.name}</p>
                        <p className="text-gray-400 text-[10px]">{s.student_display_id}</p>
                      </td>
                      <td className="px-3 py-2 text-gray-500 text-[11px] font-mono">{s.lrn}</td>
                      <td className="px-3 py-2">
                        ...
                        {s.classifications.length > 0 && (
                          <div className="flex flex-wrap gap-0.5 mt-0.5">
                            {s.classifications.map((c, ci) => (
                              <span key={ci} className="px-1 py-0.5 rounded text-[9px] font-medium bg-purple-50 text-purple-600">{c}</span>
                            ))}
                          </div>
                        )}
                      </td>
                      <td className="px-3 py-2 text-[11px]">
                        <span className={clsx(
                          ...
                          s.general_average != null
                            ? s.general_average >= 85 ? "text-green-600" : s.general_average >= 75 ? "text-amber-600" : "text-red-600"
                            : "text-gray-300"
                        )}>
                          {s.general_average != null ? s.general_average.toFixed(2) : "—"}
                        </span>
                      </td>
                      <td className="px-3 py-2 text-[11px] text-gray-500">{s.enrolled_by_name}</td>
        ...
        <div className="p-5">
          ...
          <div className="flex items-start justify-between flex-wrap gap-4 mb-5">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                Available sections: {availableSections.length} | Pending: {queue.length} students
              </p>
            </div>
            ...
            <div className="space-y-4">
                ...
                <p className="text-sm font-medium text-gray-700">
                  ...
                  {preview.filter(p => p.current_section_id != null).length > 0 && (
                    <span className="text-gray-400 ml-1">
                      · {preview.filter(p => p.current_section_id != null).length} assigned
                    </span>
                  )}
                  {preview.filter(p => p.current_section_id == null).length > 0 && (
                    <span className="text-amber-500 ml-1">
                      · {preview.filter(p => p.current_section_id == null).length} unassigned
                    </span>
                  )}
                </p>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                      <th className="text-left px-4 py-2.5 text-gray-600 font-semibold text-xs uppercase">#</th>
                      <th className="text-left px-4 py-2.5 text-gray-600 font-semibold text-xs uppercase">Student</th>
                      <th className="text-left px-4 py-2.5 text-gray-600 font-semibold text-xs uppercase">LRN</th>
                      <th className="text-left px-4 py-2.5 text-gray-600 font-semibold text-xs uppercase">Grade</th>
                      <th className="text-left px-4 py-2.5 text-gray-600 font-semibold text-xs uppercase">Program</th>
                      <th className="text-left px-4 py-2.5 text-gray-600 font-semibold text-xs uppercase">GA</th>
                      <th className="text-left px-4 py-2.5 text-gray-600 font-semibold text-xs uppercase">
                    ...
                    {preview.map((student, idx) => (
                      <tr key={student.student_id} className="hover:bg-gray-50/50">
                        ...
                        <td className="px-4 py-2.5">
                          ...
                          <p className="text-gray-400 text-xs">{student.student_display_id}</p>
                        </td>
                        <td className="px-4 py-2.5 text-gray-500 text-xs">{student.lrn}</td>
                        <td className="px-4 py-2.5">
                        ...
                        <td className="px-4 py-2.5">
                          <span className="text-xs text-gray-500">{PROGRAM_LABELS[student.program] || student.program}</span>
                          {student.classifications.length > 0 && (
                            <div className="flex flex-wrap gap-1 mt-0.5">
                              {student.classifications.map((c, ci) => (
                                <span key={ci} className="px-1.5 py-0.5 rounded text-[10px] font-medium bg-purple-50 text-purple-600">
                                  {c}
                                </span>
                              ))}
                            </div>
                        ...
                        <td className="px-4 py-2.5">
                          <span className={clsx(
                            ...
                            student.general_average != null
                              ? student.general_average >= 85 ? "text-green-600" : student.general_average >= 75 ? "text-amber-600" : "text-red-600"
                              : "text-gray-300"
                          )}>
              ...
              <div className="flex justify-end pt-2">
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-2.5 text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg transition-colors disabled:opacity-50"
                >
                  {committing ? (
                  ...
                  {commitResults && (
                    <ul className="mt-2 space-y-1 text-xs max-h-40 overflow-y-auto">
                      {commitResults.map((r: any, i: number) => (
                        <li key={i} className={r.ok ? "text-emerald-600" : "text-red-600"}>
          ...
          {preview.length === 0 && !commitMsg && !generating && (
            <div className="text-center py-12 text-gray-400">
              ...
              <p className="text-sm font-medium">No preview generated yet.</p>
              <p className="text-xs mt-1">Click <strong>"Generate Preview"</strong> to build a section assignment proposal.</p>
            </div>
          )}
