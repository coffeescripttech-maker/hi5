...
const loginSchema = z.object({
  username: z
    ...
    .max(MAX_LOGIN_USERNAME, `Username must be at most ${MAX_LOGIN_USERNAME} characters`)
...
const DEMO_ACCOUNTS: { role: string; username: string; label: string; icon: React.ElementType; desc: string; color: string }[] = [
...
export function Login() {
  ...
  const [resetErrors, setResetErrors] = useState<{ code?: string; new_password?: string; confirm?: string }>({});
  ...
  const validateAll = (): boolean => {
    ...
    if (result.success) { setFieldErrors({ username: "", password: "" }); return true; }
    const errs: { username?: string; password?: string } = {};
    result.error.issues.forEach(i => {
      const path = i.path[0];
      if (path === "username" || path === "password") { if (!errs[path]) errs[path] = i.message; }
    });
    setFieldErrors(errs);
    return false;
  };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      completeLogin(await authApi.login({ username: username.trim(), password }));
    } catch (err: unknown) {
  ...
  const handleDemoLogin = (account: typeof DEMO_ACCOUNTS[number]) => {
    ...
    authApi.login({ username: account.username, password: "password123" })
      .then(completeLogin)
      .catch(err => { setLoading(false); handleLoginError(err); });
  };
  ...
  const handleForgot = async (e: React.FormEvent) => {
    ...
    if (!result.success) {
      setForgotError(result.error.issues[0]?.message || "Please enter a valid email address.");
      return;
    }
    setForgotError("");
  ...
  const handleReset = async (e: React.FormEvent) => {
    e.preventDefault();
    const result = resetSchema.safeParse({ code: forgotCode, new_password: newPassword, confirm: confirmPassword });
    if (!result.success) {
    ...
    setForgotLoading(true);
    try {
      await authApi.resetPassword({ email: forgotEmail.trim(), code: forgotCode.trim(), new_password: newPassword });
  ...
  const LeftPanel = () => (
    <div className="hidden lg:flex lg:w-[52%] flex-col justify-between relative overflow-hidden select-none"
      ...
      <div className="absolute top-[-100px] right-[-100px] w-96 h-96 rounded-full opacity-10 pointer-events-none hi5-orb-1" style={{ background: "radial-gradient(circle, #34d399, transparent 70%)" }} />
      <div className="absolute bottom-[-80px] left-[-80px] w-72 h-72 rounded-full opacity-10 pointer-events-none hi5-orb-2" style={{ background: "radial-gradient(circle, #6ee7b7, transparent 70%)" }} />
  ...
  const MobileBanner = () => (
    <div className="lg:hidden relative flex items-center gap-3 px-4 py-3 overflow-hidden select-none shrink-0"
      style={{ backgroundImage: `url(${bgImage})`, backgroundSize: "cover", backgroundPosition: "center" }}>
      ...
      <div className="relative z-10 flex items-center gap-3 w-full">
        ...
        <div className="min-w-0">
          <p className="text-emerald-400 font-bold text-sm tracking-widest uppercase leading-tight">Hi5 Portal</p>
          <p className="text-emerald-200/60 text-xs truncate">DSPMNHS · Tinambac, Camarines Sur</p>
        </div>
        <div className="ml-auto flex-shrink-0 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full" style={{ background: "rgba(52,211,153,0.12)", border: "1px solid rgba(52,211,153,0.25)" }}>
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
          <span className="text-emerald-300 text-[11px] font-medium whitespace-nowrap">SY 2025–26</span>
        </div>
      </div>
    </div>
  );
  ...
  // ── FORGOT PASSWORD ──
  if (screen === "forgot" || screen === "forgot-reset" || screen === "forgot-sent") {
    return (
      <>
        ...
        <div className="min-h-screen flex flex-col lg:flex-row overflow-hidden">
          ...
          <div className="flex-1 flex flex-col items-center bg-gray-50 px-6 py-5 lg:py-10 overflow-y-auto">
            <div className="w-full max-w-md hi5-card my-auto">
              <button
                onClick={() => { setScreen("login"); setError(""); setForgotEmail(""); setForgotCode(""); setNewPassword(""); setConfirmPassword(""); setResetCode(""); setForgotError(""); setResetErrors({}); }}
              ...
              {screen === "forgot" && (
                <>
                  <div className="mb-8">
                    ...
                    <p className="text-gray-500 text-sm mt-1">Enter your school email address and we'll give you a password reset code.</p>
                  </div>
                  <form onSubmit={handleForgot} className="space-y-4">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          onChange={e => { setForgotEmail(e.target.value); setError(""); if (forgotError) setForgotError(""); }}
                          onBlur={() => { const r = forgotSchema.shape.email.safeParse(forgotEmail.trim()); setForgotError(r.success ? "" : r.error.issues[0]?.message || ""); }}
                          className={`w-full pl-9 pr-4 py-3 rounded-xl text-sm focus:outline-none focus:ring-2 bg-white transition shadow-sm ${
                            forgotError ? "border border-red-300 focus:ring-red-300 focus:border-red-400" : "border border-gray-200 focus:ring-emerald-400 focus:border-emerald-400"
                          }`}
                      ...
                      {forgotError && (
                        <p className="flex items-center gap-1.5 text-xs text-red-600 mt-1.5">
                          <AlertCircle size={12} className="flex-shrink-0" />{forgotError}
                        </p>
                      )}
                    </div>
                    {error && (
                    ...
                    <button type="submit" disabled={forgotLoading}
                      ...
                      {forgotLoading ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Requesting...</> : "Send Reset Code"}
                    </button>
                  </form>
                </>
              )}
              ...
              {screen === "forgot-reset" && (
                <>
                  <div className="mb-8">
                    ...
                    <h2 className="text-2xl font-extrabold text-gray-800">Create New Password</h2>
                    <p className="text-gray-500 text-sm mt-1">
                      We found an account for <span className="font-semibold text-gray-700">{forgotEmail}</span>. Enter the reset code below to continue.
                    </p>
                  </div>
                  ...
                  {resetCode && (
                    <div className="mb-4 bg-amber-50 border border-amber-200 rounded-xl p-3.5">
                      ...
                      <p className="font-mono text-2xl font-extrabold tracking-[0.3em] text-amber-800 text-center py-1 select-all">{resetCode}</p>
                      <p className="text-[11px] text-amber-600 mt-1.5">Valid for 15 minutes. (Demo mode — no email is actually sent.)</p>
                    </div>
                  )}

                  <form onSubmit={handleReset} className="space-y-4">
                    <div>
                      <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wider">6-Digit Reset Code</label>
                      <input
                        type="text" inputMode="numeric" maxLength={6} value={forgotCode}
                        onChange={e => { setForgotCode(e.target.value.replace(/\D/g, "")); if (resetErrors.code) setResetErrors(prev => ({ ...prev, code: "" })); }}
                        onBlur={() => { const r = resetSchema.shape.code.safeParse(forgotCode); setResetErrors(prev => ({ ...prev, code: r.success ? "" : r.error.issues[0]?.message || "" })); }}
                        className={`w-full px-4 py-3 rounded-xl text-sm text-center tracking-[0.3em] font-mono focus:outline-none focus:ring-2 bg-white transition shadow-sm ${
                          resetErrors.code ? "border border-red-300 focus:ring-red-300 focus:border-red-400" : "border border-gray-200 focus:ring-indigo-400 focus:border-indigo-400"
                        }`}
                        placeholder="000000" required autoComplete="one-time-code"
                      />
                      {resetErrors.code && (
                        <p className="flex items-center gap-1.5 text-xs text-red-600 mt-1.5">
                          <AlertCircle size={12} className="flex-shrink-0" />{resetErrors.code}
                        </p>
                      )}
                    </div>
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          onChange={e => { setNewPassword(e.target.value); if (resetErrors.new_password) setResetErrors(prev => ({ ...prev, new_password: "" })); }}
                          onBlur={() => { const r = resetSchema.shape.new_password.safeParse(newPassword); setResetErrors(prev => ({ ...prev, new_password: r.success ? "" : r.error.issues[0]?.message || "" })); }}
                          className={`w-full pl-9 pr-4 py-3 rounded-xl text-sm focus:outline-none focus:ring-2 bg-white transition shadow-sm ${
                            resetErrors.new_password ? "border border-red-300 focus:ring-red-300 focus:border-red-400" : "border border-gray-200 focus:ring-indigo-400 focus:border-indigo-400"
                          }`}
                      ...
                      {resetErrors.new_password && (
                        <p className="flex items-center gap-1.5 text-xs text-red-600 mt-1.5">
                          <AlertCircle size={12} className="flex-shrink-0" />{resetErrors.new_password}
                        </p>
                      )}
                    </div>
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          onChange={e => { setConfirmPassword(e.target.value); if (resetErrors.confirm) setResetErrors(prev => ({ ...prev, confirm: "" })); }}
                          onBlur={() => { const r = resetSchema.shape.confirm.safeParse(confirmPassword); setResetErrors(prev => ({ ...prev, confirm: r.success ? "" : r.error.issues[0]?.message || "" })); }}
                          className={`w-full pl-9 pr-4 py-3 rounded-xl text-sm focus:outline-none focus:ring-2 bg-white transition shadow-sm ${
                            resetErrors.confirm ? "border border-red-300 focus:ring-red-300 focus:border-red-400" : "border border-gray-200 focus:ring-indigo-400 focus:border-indigo-400"
                          }`}
                      ...
                      {resetErrors.confirm && (
                        <p className="flex items-center gap-1.5 text-xs text-red-600 mt-1.5">
                          <AlertCircle size={12} className="flex-shrink-0" />{resetErrors.confirm}
                        </p>
                      )}
                    </div>
                    {forgotError && (
                    ...
                    <button type="submit" disabled={forgotLoading}
                      className="w-full py-3 rounded-xl font-semibold text-sm text-white transition-all duration-200 hover:brightness-105 active:scale-[0.98] disabled:opacity-70 flex items-center justify-center gap-2"
                      style={{ background: "linear-gradient(135deg, #059669 0%, #0d9488 100%)", boxShadow: "0 4px 20px rgba(5,150,105,0.3)" }}>
                      {forgotLoading ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Resetting...</> : <><Lock size={15} /> Reset Password</>}
                    </button>
                  </form>
                </>
              )}
              ...
              {screen === "forgot-sent" && (
                <div className="text-center">
                  ...
                  <p className="text-gray-500 text-sm mb-1">Your password has been changed successfully for:</p>
                  <p className="text-emerald-600 font-semibold text-sm mb-6">{forgotEmail}</p>
                  <p className="text-gray-400 text-xs mb-6">You can now sign in with your new password.</p>
                  <button onClick={() => { setScreen("login"); setForgotEmail(""); setForgotCode(""); setNewPassword(""); setConfirmPassword(""); setResetCode(""); setError(""); }}
        ...
        <div className="flex-1 flex flex-col items-center bg-gray-50 px-4 sm:px-6 py-5 lg:py-8 overflow-y-auto">
          <div className="w-full max-w-md my-auto">
            ...
            <div className="hi5-card hi5-stagger-1 mb-7 flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-extrabold text-gray-800 leading-tight">Welcome back</h2>
                <p className="text-gray-500 text-sm">Sign in to the Hi5 Portal</p>
              </div>
            </div>
            ...
            <form onSubmit={handleLogin}
              ...
              <div>
                <label htmlFor="login-username" className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wider">Username</label>
                <div className="relative group">
                  <User size={15} className={`absolute left-3.5 top-1/2 -translate-y-1/2 transition group-focus-within:text-emerald-600 ${fieldErrors.username ? "text-red-400" : "text-gray-400"}`} />
                  <input id="login-username" type="text" value={username}
                    onChange={e => { setUsernameInput(e.target.value); if (fieldErrors.username) validateField("username", e.target.value); }}
                    onBlur={e => validateField("username", e.target.value)}
                    className={`w-full pl-9 pr-4 py-3 rounded-xl text-sm focus:outline-none focus:ring-2 bg-white transition shadow-sm ${
                      fieldErrors.username ? "border border-red-300 focus:ring-red-300 focus:border-red-400" : "border border-gray-200 focus:ring-emerald-400/70 focus:border-emerald-400"
                    }`}
                    placeholder="Enter your username" required autoComplete="username"
                    onKeyDown={e => e.key === "Enter" && password && document.getElementById("login-password")?.focus()} />
                </div>
              ...
              <div>
                <label htmlFor="login-password" className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wider">Password</label>
                <div className="relative group">
                  <Lock size={15} className={`absolute left-3.5 top-1/2 -translate-y-1/2 transition group-focus-within:text-emerald-600 ${fieldErrors.password ? "text-red-400" : "text-gray-400"}`} />
                  <input id="login-password" type={showPass ? "text" : "password"} value={password}
                    ...
                    className={`w-full pl-9 pr-11 py-3 rounded-xl text-sm focus:outline-none focus:ring-2 bg-white transition shadow-sm ${
                      fieldErrors.password ? "border border-red-300 focus:ring-red-400 focus:border-red-400" : "border border-gray-200 focus:ring-emerald-400/70 focus:border-emerald-400"
                    }`}
              ...
              {/* Errors */}
              {error && (
                <div className="hi5-card flex items-center gap-2 rounded-xl px-4 py-3 text-sm text-red-700 bg-red-50 border border-red-200 shadow-sm" style={{ animationDuration: ".25s" }}>
              ...
              <button type="submit" disabled={loading || isLockedOut}
                ...
                style={{ background: isLockedOut ? "#9ca3af" : "linear-gradient(135deg, #059669 0%, #0d9488 100%)", boxShadow: isLockedOut ? "none" : "0 8px 24px rgba(5,150,105,0.35)" }}>
            ...
            <p className="text-center text-xs text-gray-300 mt-4 mb-2">© 2026 Hi5 Portal · DSPMNHS</p>
          </div>
        </div>
      </div>
    </>
