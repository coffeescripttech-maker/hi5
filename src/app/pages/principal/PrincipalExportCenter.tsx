  ...
  return [
    {
      ...
      text: (school?.school_name || "DON SERVILLANO PLATON MEMORIAL NATIONAL HIGH SCHOOL").toUpperCase(),
      bold: true,
      fontSize: 14,
      margin: [0, 0, 0, 2]
    },
    {
      alignment: "center" as const,
      text: (school?.division || "Schools Division of Camarines Sur") + " · Sta. Cruz, Tinambac, Camarines Sur",
      fontSize: 9,
      margin: [0, 0, 0, 2]
    },
    {
...
export function PrincipalExportCenter() {
  ...
  const exportSF5 = () =>
    runExport("sf5", `SF5-${sectionName || "All"}-${syLabel}`, async () => {
      ...
      const header = buildHeaderRows(school, syLabel, "SF5 — Report on Promotion");
      return {
        content: [
          ...header,
          {
            table: {
              ...
              body: [
                ...
                ...data.students.map(s => [
                  ...
                  s.name,
                  s.section_name,
                  s.general_average === null ? "INC" : String(s.general_average),
                  s.promotion_status
                ])
              ]
            },
  ...
  const exportSF1 = () =>
    runExport("sf1", `SF1-${sectionName || "All"}-${syLabel}`, async () => {
      ...
      const header = buildHeaderRows(school, syLabel, "SF1 — School Register (First Grading)");
      return {
        content: [
          ...header,
          {
    ...
    runExport("reports", `School-Reports-${syLabel}`, async () => {
      ...
      return {
        content: [
          ...
          { text: "1. Enrollment by Grade & Sex", bold: true, fontSize: 10, margin: [0, 0, 0, 4] },
          {
            table: {
              headerRows: 1,
          ...
          { text: "2. Grade Submission Status", bold: true, fontSize: 10, margin: [0, 14, 0, 4] },
          {
            table: {
              headerRows: 1,
              widths: ["*", "auto", "auto"],
          ...
          { text: `3. Promotion Records (${promos.length})`, bold: true, fontSize: 10, margin: [0, 14, 0, 4] },
          ...promos
            .slice(0, 25)
            .map(p =>
  ...
  const exportData = () =>
    runExport("data", `School-Wide-Data-${syLabel}`, async () => {
      const dist = await gradesApi.getDistribution({ school_year_id: selectedSY });
      const subjectRows = dist.subjects.map(s => [
        s.subject_name,
        String(s.total_students),
        s.mean_grade === null ? "—" : s.mean_grade.toFixed(2),
        s.pass_rate === null ? "—" : `${s.pass_rate.toFixed(1)}%`
      ]);
      return {
        content: [
          ...buildHeaderRows(school, syLabel, "School-wide Data — Grade Distribution"),
          {
            text: `Overall pass rate: ${dist.overall_pass_rate.toFixed(1)}% across ${dist.total_students} students`,
            fontSize: 9,
            margin: [0, 0, 0, 6]
          },
          {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <h3 className="font-bold text-gray-800 text-sm">SF1 — School Register</h3>
            </div>
            <p className="text-xs text-gray-400 mb-4">First-grading register for the selected section.</p>
            <ExportButton busy={exporting === "sf1"} onClick={exportSF1} label="Export SF1 PDF" />
          </div>
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <div className="flex items-center gap-2 mb-1.5">
              <GraduationCap size={16} className="text-purple-700" />
              <h3 className="font-bold text-gray-800 text-sm">SF5 — Report on Promotion</h3>
            </div>
            <p className="text-xs text-gray-400 mb-4">Promotion summary with general weighted average.</p>
            <ExportButton busy={exporting === "sf5"} onClick={exportSF5} label="Export SF5 PDF" />
          </div>
        </div>
      )}
      ...
      {effective === "reports" && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="font-bold text-gray-800 text-sm">Consolidated School Reports</h3>
          </div>
          <p className="text-xs text-gray-400 mb-2">
            Enrollment by grade & sex, grade submission status, and promotion records for {syLabel || "the selected school year"}.
          </p>
          <div className="flex items-center gap-2 text-[11px] text-gray-400 mb-4">
            <Calendar size={12} /> Generated with official school header
          </div>
          <ExportButton busy={exporting === "reports"} onClick={exportReports} label="Export Reports PDF" />
        </div>
      )}
      ...
      {effective === "data" && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="font-bold text-gray-800 text-sm">School-wide Grade Distribution</h3>
          </div>
          <p className="text-xs text-gray-400 mb-2">
            Per-subject student counts, mean grades, and pass rates across the school for {syLabel || "the selected school year"}.
          </p>
          <ExportButton busy={exporting === "data"} onClick={exportData} label="Export Data PDF" />
        </div>
      )}
    </div>
  );
